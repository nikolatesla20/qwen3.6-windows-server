"""Launch vLLM serving Qwen3.6-27B in thinking mode tuned for coding
workloads at 127k context.

Sampler defaults match Unsloth's recommended Thinking-precise-coding
row (temperature 0.6, top_p 0.95, top_k 20, min_p 0.0). Thinking is
left enabled so the model writes a <think> block before answering, but
the sampler is tightened from the general thinking-mode default
(temp 1.0) for cleaner code output. Per-request sampler params still
override these defaults.

Use this snapshot for code review, debugging, and architecture-level
reasoning where the <think> block adds value but you still want
tighter, code-friendly sampling.
"""
...
# Unsloth's Thinking-precise-coding sampler row.
...
# enable_thinking is left at the default (true) so the model still
# emits a <think> block per response.
CHAT_TEMPLATE_KWARGS = '{"preserve_thinking": false}'
MODE_LABEL = "Thinking, precise coding"
