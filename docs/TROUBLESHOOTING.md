...
| Boot fails with `ValueError: CUDA_LIB_PATH is not set` from `flashinfer/jit/__init__.py` during EngineCore init | vLLM 0.19 unconditionally imports flashinfer in `topk_topp_sampler.py` regardless of `--attention-backend`. flashinfer's Windows path raises at import time if `CUDA_LIB_PATH` is missing, even though shipped snapshots use TRITON_ATTN and never trigger flashinfer JIT. | **Auto-fixed since v0.1.15**, the snapshots probe `CUDA_PATH`, `CUDA_HOME`, and standard NVIDIA install dirs and set `CUDA_LIB_PATH` before launching vLLM. If no CUDA Toolkit is installed, a placeholder is set so the import check passes (TRITON_ATTN never needs the real CUDA libs). Manual fallback if you're on an older release: `set CUDA_LIB_PATH=C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.4` (adjust path) before running `start.bat`. |
